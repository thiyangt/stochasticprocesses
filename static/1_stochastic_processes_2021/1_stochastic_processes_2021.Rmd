...
ggplot(df, aes(x=x, y=y)) + geom_point()  + scale_x_discrete(breaks=c(1, 0, 10)) + geom_line()
```
...
Random variables $X_1, X_2, X_3, ...$ form a stochastic process ordered by the **discrete index set** $\{1, 2, 3,...\}$.

Notation: $\{X_t: t=1, 2, 3, ...\}$.
...
The number of customers in queue in front of an ATM for at the end of each hour of a day. What type of stochastic process is this?
